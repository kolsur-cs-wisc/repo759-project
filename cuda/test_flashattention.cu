...
inline size_t getIndex(size_t b, size_t t, size_t c, size_t nh, size_t T, size_t C, size_t NH)
{
   return b * T * C * NH + t * C * NH + nh * C + c;
}

inline size_t getIndexOutput(size_t b, size_t t, size_t c, size_t nh, size_t T, size_t C, size_t NH)
{
   // B *NH *T *C
...
void permuteTensor(const float *input, float *output,
                   size_t B, size_t T, size_t C, size_t NH)
{
      ...
      for (size_t nh = 0; nh < NH; ++nh)
      {
         for (size_t t = 0; t < T; ++t)
         {
            for (size_t c = 0; c < C; ++c)
            {
               ...
               // std::cout << "Input Index = " << inputIdx << " : " << outputIdx << std::endl;
...
// Function to permute dimensions of the tensor
void permuteOutput(const float *input, float *output,
                   size_t B, size_t T, size_t C, size_t NH)
{
...
int main()
{
   ...
   dim3 grid_dim(B, NH);
   // flash_attention_kernel(const float *Q, const float *K, const float *V, float *O, unsigned int N, unsigned int d, float scaling_factor, float *l, float *m, int NH);
   size_t sram_size = Br * C * 3 * sizeof(float) + Br * Br * sizeof(float);
   flash_attention_kernel<<<grid_dim, Br, sram_size>>>(d_Q, d_K, d_V, d_flash, T, C, factor, d_l, d_m, NH);
   cudaMemcpy(h_output_flash, d_flash, B * NH * T * C * sizeof(float), cudaMemcpyDeviceToHost);
